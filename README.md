...
<head>
    ...
    <style>
        ...
        /* ==============================
           FIXED SCREEN CSS (NO SCROLLING/DRAGGING)
           ============================== */
        html, body {
            ...
            user-select: none; /* Prevent text selection & copying */
        ...
        .content { 
            flex-grow: 1; 
            padding: 20px; 
            overflow-y: auto; /* Only internal scrolling allowed */
            overflow-x: hidden;
            padding-bottom: 90px;
        ...
        input, select {
            ...
            background: #fafafa;
            transition: 0.3s;
            /* Prevent styling glitches on fixed body */
...
<body>
        ...
        <main class="content">
            ...
            <section id="earn-page" class="page">
                <div class="card">
                    ...
                    <!-- RESTORED 2 BUTTONS -->
                    <button class="btn-primary" id="watch-ad-btn-1" onclick="watchAd(1)">
                        Watch Ad 1 (Reward ৳15)
                        <span class="material-icons">play_circle_filled</span>
                    </button>
                    
                    <button class="btn-primary" style="background: #2979ff;" id="watch-ad-btn-2" onclick="watchAd(2)">
    ...
    <script>
        ...
        window.watchAd = (btnId) => {
            ...
            const btnIdStr = btnId === 1 ? '1' : '2';
            const btn = document.getElementById(`watch-ad-btn-${btnIdStr}`);
            const originalText = btn.innerHTML;
            
            btn.disabled = true;
            btn.innerHTML = 'Loading Ad...';

            // Lock the OTHER button temporarily
            const otherBtnId = btnId === 1 ? '2' : '1';
            const otherBtn = document.getElementById(`watch-ad-btn-${otherBtnId}`);
            if(otherBtn) otherBtn.disabled = true;

            if (typeof show_10345622 === 'function') {
                show_10345622('pop') 
                    .then(() => {
                        giveAdReward(btn, originalText, otherBtn);
                    })
                    .catch(e => {
        ...
        function giveAdReward(btn, originalText, otherBtn) {
            ...
            btn.disabled = false;
            btn.innerHTML = originalText;
            if(otherBtn) otherBtn.disabled = false; // Unlock other button
            saveData();
        }
