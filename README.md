    ...
    <script>
        ...
        window.watchAd = (btnId) => {
            ...
            if (typeof show_10345622 === 'function') {
                show_10345622('pop')
                    .then(() => {
                        // SUCCESS PATH: Add Reward
                        giveAdReward(btn, originalText);
                    })
                    .catch(e => {
                        // ERROR PATH: Reset button
                        console.error("Ad Error:", e);
                        tg.showAlert('Ad failed to load or network error. Please try again.');
                        btn.disabled = false;
                        btn.innerHTML = originalText;
                    });
            } else {
                // FALLBACK SIMULATION (If SDK is missing)
                setTimeout(() => {
                    giveAdReward(btn, originalText);
                }, 2000);
            }
        ...
        function giveAdReward(btn, originalText) {
            ...
            tg.showAlert(`Success! You earned ৳${CONFIG.REWARD_PER_AD}`);
